...
const ManagementDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [ordersRes, billRequestsRes, feedbackRes, billHistoryRes] = await Promise.all([
        fetch('http://192.168.0.191:4000/api/orders'),
        fetch('http://192.168.0.191:4000/api/bills/requests'),
        fetch('http://192.168.0.191:4000/api/feedback'),
        fetch('http://192.168.0.191:4000/api/bills')
      ]);
  ...
  const connectWebSocket = () => {
    ...
    ws.current = new WebSocket('ws://192.168.0.191:4000');
    ...
    ws.current.onmessage = (event) => {
      try {
        ...
        switch (data.type) {
          ...
          case 'BILL_CREATED':
            if (data.tableNumber) {
              setBillRequests(prevRequests => 
                prevRequests.filter(request => request.tableNumber !== data.tableNumber)
              );
            }
            fetchData();
            break;
  ...
  const handleAcceptOrder = async (orderId: string) => {
    try {
      await fetch(`http://192.168.0.191:4000/api/orders/${orderId}/accept`, {
        method: 'POST',
      });
      fetchData();
    } catch (error) {
  ...
  const handleBillRequest = async (tableNumber: number) => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch('http://192.168.0.191:4000/api/bills', {
        method: 'POST',
      ...
      setBillRequests(prevRequests => 
        prevRequests.filter(request => request.tableNumber !== tableNumber)
      );
  ...
  const handleBillServed = async (billId: string) => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`http://192.168.0.191:4000/api/bills/${billId}/payment`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      await fetch(`http://192.168.0.191:4000/api/orders/${orderId}/delete`, {
        method: 'DELETE',
      });
      fetchData();
    } catch (error) {
  ...
  const handleCompleteOrder = async (orderId: string) => {
    try {
      await fetch(`http://192.168.0.191:4000/api/orders/${orderId}/complete`, {
        method: 'POST',
      });
      fetchData();
    } catch (error) {
  ...
  const handleCancelOrder = async (orderId: string) => {
    try {
      await fetch(`http://192.168.0.191:4000/api/orders/${orderId}/cancel`, {
        method: 'POST',
      });
      fetchData();
    } catch (error) {
