...
const MenuView: React.FC = () => {
  ...
  const handleRequestBill = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.0.191:4000/api/orders?tableNumber=${tableNumber}`);
      const orders = await response.json();

      // Update all orders to mark them as bill requested
      await Promise.all(orders.map(order => 
        fetch(`http://192.168.0.191:4000/api/orders/${order._id}/status`, {
          method: 'PATCH',
  ...
  const handleFeedbackSubmit = async () => {
    try {
      ...
      if (feedbackData.name || feedbackData.phone || feedbackData.email || feedbackData.message) {
        await fetch('http://192.168.0.191:4000/api/feedback', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...feedbackData,
  ...
  return (
    <Box sx={{ width: '100%', overflow: 'hidden', bgcolor: '#f8f8f8' }}>
      ...
      {/* Quick Info Bar */}
        ...
        <Box sx={{ 
          ...
          <Typography variant="body2">+91 70832 06131</Typography>
        </Box>
      </Box>
